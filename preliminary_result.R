...
# Padroniza as variáveis quantitativas:
...
#############################################
# (Exemplo) Vamos construir uma árvore de classificação #
...
# Avaliando uma árvore - Predição e Métricas de Performance: (Exemplo) #
...
#Calculando a sua profundidade:
nodes <- as.numeric(rownames(arvore$frame))
...
rpart.plot::rpart.plot(arvore_poda,
                       #box.palette = paleta, extra = 106, fallen.leaves = FALSE, tweak = 1.5) # Paleta de cores
                       box.palette = paleta, extra = 106, tweak = 1.8) # Paleta de cores
